(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(magit-status-sections-hook
   '(magit-insert-status-headers magit-insert-local-branches magit-insert-merge-log magit-insert-rebase-sequence magit-insert-am-sequence magit-insert-sequencer-sequence magit-insert-bisect-output magit-insert-bisect-rest magit-insert-bisect-log magit-insert-untracked-files magit-insert-unstaged-changes magit-insert-staged-changes magit-insert-stashes magit-insert-unpushed-to-pushremote magit-insert-unpushed-to-upstream-or-recent magit-insert-unpulled-from-pushremote magit-insert-unpulled-from-upstream)))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(avy-goto-char-timer-face ((t (:background "red" :foreground nil))))
 '(avy-lead-face ((t (:background "red" :foreground "white"))))
 '(avy-lead-face-0 ((t (:background "red" :foreground "white"))))
 '(avy-lead-face-1 ((t (:background "red" :foreground "white"))))
 '(avy-lead-face-2 ((t (:background "red" :foreground "white"))))
 '(custom-saved ((t (:background "color-22" :foreground "#b6e63e" :weight bold))))
 '(custom-state ((t (:background "color-22" :foreground "#b6e63e"))))
 '(eros-result-overlay-face ((t (:background nil))))
 '(font-lock-comment-face ((t (:foreground "color-246"))))
 '(ivy-modified-buffer ((t (:foreground nil :inherit font-lock-doc-face))))
 '(magit-diff-added ((t (:extend t :background "color-22" :foreground "color-114"))))
 '(magit-diff-added-highlight ((t (:extend t :background "color-22" :foreground "#9ac334" :weight bold))))
 '(magit-diff-base ((t (:extend t :background "color-58" :foreground "#ca7818"))))
 '(magit-diff-base-highlight ((t (:extend t :background "color-58" :foreground "#fd971f" :weight bold))))
 '(magit-dimmed ((t (:foreground nil :inherit font-lock-comment-face))))
 '(magit-hash ((t (:foreground nil :inherit font-lock-comment-face))))
 '(magit-refname ((t (:foreground nil :inherit font-lock-comment-face))))
 '(show-paren-match ((t (:background "brightblack" :foreground nil))))
 '(smerge-lower ((t (:extend t :background "color-22"))))
 '(swiper-line-face ((t (:background "color-235" :foreground nil)))))
